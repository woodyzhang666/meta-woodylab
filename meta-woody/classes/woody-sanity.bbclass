...
python woody_update_bblayersconf() {
    current_version = int(d.getVar('WOODY_BBLAYERS_CONF_VERSION', True) or -1)
    ...
    if success:
        ...
        #sanity_conf_update(bblayers_fn, lines, 'WOODY_BBLAYERS_CONF_VERSION', current_version)
        index, line = sanity_conf_find_line(r'^WOODY_BBLAYERS_CONF_VERSION', lines)
        lines[index] = 'WOODY_BBLAYERS_CONF_VERSION = "%d"\n' % current_version
...
BBLAYERS_CONF_UPDATE_FUNCS += "conf/bblayers.conf:WOODY_BBLAYERS_CONF_VERSION:REQUIRED_WOODY_BBLAYERS_CONF_VERSION:woody_update_bblayersconf"
